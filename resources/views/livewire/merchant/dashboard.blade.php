...
<div>
        ...
        <div class="balance-hero px-6 py-7 lg:px-10 lg:py-10">
            ...
            <div class="relative">
                <div class="flex justify-between items-start">
                    <div>
                        <div class="text-xs font-semibold uppercase tracking-wider mb-2" style="color: rgba(255,255,255,0.5);">{{ __('merchant.dashboard.available_balance') }}</div>
                        ...
                        <div class="text-xs mt-2" style="color: rgba(255,255,255,0.45);">
                            + {{ FormatService::kmf($balance['frozenBalance']) }} {{ __('merchant.dashboard.frozen_suffix') }}
                        </div>
                        @endif
                    </div>
                ...
                <div class="grid grid-cols-3 gap-3 mt-6 pt-5" style="border-top: 1px solid rgba(255,255,255,0.1);">
                    <div>
                        <div class="text-xs uppercase tracking-wider mb-1" style="color: rgba(255,255,255,0.45); font-weight: 600;">{{ __('merchant.dashboard.today') }}</div>
                        <div class="font-mono font-semibold" style="font-size: 14px; color: #4ade80;">+{{ FormatService::kmf($balance['todayIn']) }}</div>
                    </div>
                    <div>
                        <div class="text-xs uppercase tracking-wider mb-1" style="color: rgba(255,255,255,0.45); font-weight: 600;">{{ __('merchant.dashboard.sales') }}</div>
                        <div class="font-mono font-semibold" style="font-size: 14px; color: #fff;">{{ $balance['todayCount'] }}</div>
                    </div>
                    <div>
                        <div class="text-xs uppercase tracking-wider mb-1" style="color: rgba(255,255,255,0.45); font-weight: 600;">{{ __('merchant.dashboard.week') }}</div>
                ...
                <div class="flex gap-3 mt-6">
                    ...
                    <a wire:navigate href="{{ route('merchant.statement') }}" style="flex: 1; height: 44px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.12); border-radius: 10px; color: rgba(255,255,255,0.85); font-weight: 600; font-size: 13.5px; display: flex; align-items: center; justify-content: center; gap: 6px; text-decoration: none; cursor: pointer;">
                        <x-icon name="doc" class="w-4 h-4"/>{{ __('merchant.dashboard.statement') }}
                    </a>
                    <a wire:navigate href="{{ route('merchant.terminals') }}" style="flex: 1; height: 44px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.12); border-radius: 10px; color: rgba(255,255,255,0.85); font-weight: 600; font-size: 13.5px; display: flex; align-items: center; justify-content: center; gap: 6px; text-decoration: none; cursor: pointer;">
                        <x-icon name="device" class="w-4 h-4"/>{{ __('merchant.dashboard.terminals') }}
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="px-5 lg:px-0 mb-5 lg:col-span-2 lg:order-1">
        <div class="card p-5">
            <div class="flex justify-between items-center mb-4">
                <div class="section-title">{{ __('merchant.dashboard.sales_today_kmf') }}</div>
                <span class="pill pill-success">{{ __('merchant.dashboard.live') }}</span>
            </div>
    ...
    <div class="px-5 lg:px-0 mb-5 lg:col-span-1 lg:order-2">
        <div class="card p-5">
            <div class="flex justify-between items-center mb-4">
                <div class="section-title">{{ __('merchant.dashboard.terminals_section') }}</div>
                <a wire:navigate href="{{ route('merchant.terminals') }}" style="font-size: 12.5px; font-weight: 600; color: var(--color-brand); text-decoration: none;">{{ __('merchant.dashboard.view_all') }}</a>
            </div>
            ...
            <div style="font-size: 12px; color: var(--color-ink-low); margin-bottom: 14px;">{{ __('merchant.dashboard.online') }}</div>
    ...
    <div class="px-5 lg:px-0 lg:col-span-3 lg:order-3">
        <div class="flex justify-between items-center mb-3">
            <div class="section-title">{{ __('merchant.dashboard.recent_payments') }}</div>
            <a wire:navigate href="{{ route('merchant.transactions') }}" style="font-size: 12.5px; font-weight: 600; color: var(--color-brand); text-decoration: none;">{{ __('merchant.dashboard.view_all') }}</a>
        </div>

        @if(empty($transactions))
        <div class="empty-state">
            <x-icon name="list" class="w-10 h-10 mb-3" style="color: var(--color-border-hi);"/>
            <div style="font-size: 15px; font-weight: 600;">{{ __('merchant.dashboard.no_tx_yet') }}</div>
        </div>
        @else
        <div class="card overflow-hidden">
            ...
            <div class="hidden lg:grid grid-cols-12 px-4 py-2" style="background: var(--color-surface-alt); border-bottom: 1px solid var(--color-border);">
                <div class="col-span-4 section-title">{{ __('merchant.dashboard.counterparty') }}</div>
                <div class="col-span-2 section-title">{{ __('merchant.dashboard.operator') }}</div>
                <div class="col-span-2 section-title">{{ __('merchant.dashboard.terminal') }}</div>
                <div class="col-span-1 section-title">{{ __('merchant.dashboard.status') }}</div>
                <div class="col-span-3 section-title text-right">{{ __('merchant.dashboard.amount') }}</div>
            </div>
